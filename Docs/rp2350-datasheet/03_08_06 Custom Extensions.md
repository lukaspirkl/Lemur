...
The interrupt priority array [MEIPRA](#page-330-1) implements a four-bit field per interrupt. In hardware, numerically higher (unsigned) [MEIPRA](#page-330-1) values have higher priority, taking precedence over lower-priority interrupts. The irq\_set\_priority() SDK function uses the opposite convention, with lower numeric values indicating greater precedence. This section uses the hardware numbering.
