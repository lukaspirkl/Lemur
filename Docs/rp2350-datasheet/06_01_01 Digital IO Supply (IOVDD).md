...
If the digital IO is powered at a nominal 1.8 V, the IO input thresholds should be adjusted by setting the [VOLTAGE\\_SELECT](#page-784-0) register to 1. [VOLTAGE\\_SELECT](#page-784-0) is set to 0 by default, which results in input thresholds that are valid for a nominal IO voltage between 2.5 V and 3.3 V. See [Chapter 9](#page-584-0) for details.
