...
The pad output disable bit [\(GPIO0.](#page-784-1)OD) for each pad is clear at reset, but the IO muxing is reset to the null function,

which ensures that the output buffer is high-impedance.

#### **IMPORTANT**

The pad reset state is different from RP2040, which only disables digital inputs on GPIOs 26 through 29 (as of version B2) and does not have isolation latches. Applications must enable the pad input ([GPIO0](#page-784-1).IE = 1) and disable pad isolation latches ([GPIO0.](#page-784-1)ISO = 0) before using the pads for digital I/O. The gpio\_set\_function() SDK function performs these tasks automatically.
