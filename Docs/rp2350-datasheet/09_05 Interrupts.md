...
There are enable, status, and force registers for three interrupt destinations: proc 0, proc 1, and dormant\_wake. For proc 0 the registers are enable [\(PROC0\\_INTE0\)](#page-702-0), status ([PROC0\\_INTS0](#page-714-0)), and force [\(PROC0\\_INTF0\)](#page-708-0). Dormant wake is used to wake the ROSC or XOSC up from dormant mode. See [Section 6.5.6.2](#page-489-0) for more information on dormant mode.
...
Summary registers can be used to quickly check for pending GPIO interrupts. See [IRQSUMMARY\\_PROC0\\_NONSECURE0](#page-688-0) for an example.
