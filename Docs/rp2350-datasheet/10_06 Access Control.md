...
For each GPIO, including the QSPI and USB DP/DM pins, a bit in the [GPIO\\_NSMASK0](#page-828-0) and [GPIO\\_NSMASK1](#page-828-1) register can be set to make that GPIO accessible to both the Secure and Non-secure domains, or clear to make it Secure-only. This has system-wide implications, controlling:
...
Finally, the [FORCE\\_CORE\\_NS](#page-827-0) register makes core 1's bus accesses appear to be Non-secure at system level. This supports schemes where all Secure services run on core 0, and therefore core 1 should not be able to access Secure hardware.
