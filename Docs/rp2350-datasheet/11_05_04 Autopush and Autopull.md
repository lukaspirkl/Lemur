...
[Figure 49](#page-906-0) shows how the state machine executes the example program. Initially the OSR is empty, so the state machine stalls on the first OUT instruction. Once data is available in the TX FIFO, the state machine transfers this into the OSR. On the next cycle, the OUT can execute using the data in the OSR (in this case, transferring this data to the X scratch register), and the state machine simultaneously refills the OSR with fresh data from the FIFO. Since every IN instruction immediately fills the ISR, the ISR remains empty, and IN transfers data directly from scratch X to the RX FIFO.

*Figure 49. Execution of auto\_push\_pull program. The state machine stalls on an OUT until data has travelled through the TX FIFO into the OSR. Subsequently, the OSR is refilled simultaneously with each OUT operation (due to bit count of 32), and IN data bypasses the ISR and goes straight to the RX FIFO. The state machine stalls again when the FIFO has drained, and the OSR is once again empty.*

<span id="page-906-0"></span>![](_page_906_Figure_2.jpeg)
...
- <sup>A</sup>PULL instruction or autopull clears the OSR counter to 0
- <sup>A</sup>PUSH instruction or autopush clears the ISR counter to 0
- <sup>A</sup>MOV OSR, x or MOV ISR, x clears the OSR or ISR shift counter to 0, respectively
- <sup>A</sup>OUT ISR, n instruction sets the ISR shift counter to <sup>n</sup>
...
Autopush must not be enabled when SHIFTCTRL\_FJOIN\_RX\_PUT or SHIFTCTRL\_FJOIN\_RX\_PUTGET is set. Its operation in this state is undefined.

#### <span id="page-907-0"></span>**11.5.4.2. Autopull Details**
