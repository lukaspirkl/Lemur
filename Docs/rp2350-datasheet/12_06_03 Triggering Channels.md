...
- A write to the [MULTI\\_CHAN\\_TRIGGER](#page-1132-0) register (can trigger multiple channels at once).

Each trigger mechanism covers different use cases. For example, trigger registers are simple and efficient when

configuring and starting a channel in an interrupt service routine because the channel is triggered by the last configuration write. CHAIN\_TO allows one channel to callback to another channel, which can then reconfigure the first channel. [MULTI\\_CHAN\\_TRIGGER](#page-1132-0) allows software to simply start a channel without touching any of its configuration registers.

Once triggered, the channel sets its CTRL.BUSY flag to indicate it is actively scheduling transfers. This remains set until the transfer count reaches zero, or the channel is aborted via the [CHAN\\_ABORT](#page-1134-1) register ([Section 12.6.8.3](#page-1104-1)).
...
#### <span id="page-1096-0"></span>**12.6.3.1. Aliases and Triggers**
...
- The value 0 is written to the trigger register (useful for ending control block chains, see null triggers [\(Section](#page-1097-3) [12.6.3.3\)](#page-1097-3))
- The bus access has a security level lower than the channel's security level ([Section 12.6.6.1](#page-1100-0))

#### <span id="page-1097-2"></span>**12.6.3.2. Chaining**
...
The [Section 12.6.9](#page-1105-0) goes into more detail on the possibilities of chain triggers in the real world.

#### <span id="page-1097-3"></span>**12.6.3.3. Null Triggers and Chain Interrupts**

As mentioned in [Section 12.6.3.1,](#page-1096-0) writing all-zeroes to a trigger register does *not* start the channel. This is called a null trigger, and it has two purposes:
...
The channel CTRL register has a field called IRQ\_QUIET. Its default value is 0. When this set to 1, channels generate an interrupt when they receive a null trigger, but not on normal completion of a transfer sequence. The interrupt is generated by the channel which receives the trigger.
