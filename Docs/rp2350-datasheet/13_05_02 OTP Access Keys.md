...
- <sup>a</sup>**read key** index 1-7, or 0 if there is no read key
- <sup>a</sup>**write key** index 1-7 or 0 if there is no write key
...
There are six 128-bit access keys stored in the OTP. Keys 5 and 6 also function as the Secure debug access key and Non-secure debug access key, respectively. See [Section 3.5.9.2](#page-92-1) for information on how the debug keys affect external debug access.
