...
When you read an OTP value through an ECC alias [\(Section 13.1\)](#page-1265-1), ECC recalculates the six parity bits based on the value read from the OTP row. Then, ECC XORs the original six parity bits with the newly-calculated parity bits. This generates 6 new bits:
