# <span id="page-867-1"></span>**10.9.2. Trigger Response**

When any of the detectors fires, the corresponding bit in the [TRIG_STATUS](#page-870-0) is set. If the glitch detector block is armed, this detector event also resets almost all logic in the switched core domain. The glitch detector is armed if:
...
After a glitch detector-initiated reset, the [CHIP_RESET](#page-467-0).HAD_GLITCH_DETECT flag is set so that software can diagnose that the last reset was caused by a glitch detector trigger. Check the [TRIG_STATUS](#page-870-0) register to see which detector fired. This can be useful for tuning the thresholds of individual detectors.
