# <span id="page-1141-0"></span>**12.7.3. Architecture**
...
clk_sys must also be running at > 48MHz. See [RP2350-E12](#page-1365-1).
...
- 2. nop for some clk_sys cycles to ensure that at least one clk_usb cycle passes. Consider a scenario where clk_sys runs at 125MHz and clk_usb runs at 48MHz. Because , you should issue 3 nop instructions between the writes to guarantee that at least one clk_usb cycle has passed.
...
For host interrupt and isochronous packets on EPx, the buffer full bit will be set on completion even if the transfer was unsuccessful. To determine the error, read the error bits in the [SIE_STATUS](#page-1161-0) register.
...
bits for EP0 are gated with two bits in the [EP_STALL_ARM](#page-1167-1) register. These bits are cleared when a setup packet is received. This means that to send a stall on EP0, you must set both the stall bit in the buffer control register and the appropriate bit in [EP_STALL_ARM](#page-1167-1).
...
Finally, [SIE_STATUS](#page-1161-0).SETUP_REC is set to indicate that a setup packet has been received. This will trigger an interrupt if the programmer has enabled the SETUP_REC interrupt (see [INTE](#page-1173-0)).
...
- 1. If STALL is set in the buffer control register (and if EP0, the appropriate [EP_STALL_ARM](#page-1167-1) bit is set), send a STALL response and go to idle.
...
- 1. If this was the last buffer in the transfer (i.e. if the LAST_BUFFER bit in the buffer control register was set), set [SIE_STATUS](#page-1161-0).TRANS_COMPLETE.
- 2. If the endpoint is double buffered, flip the buffer select to the other buffer.
- 3. Set a bit in [BUFF_STATUS](#page-1162-0) to indicate the buffer is done. When handling this event, the programmer should read [BUFF_CPU_SHOULD_HANDLE](#page-1164-0) to see if it is buffer 0 or buffer 1 that is finished. If the endpoint is double-buffered, both buffers could be done. The cleared [BUFF_STATUS](#page-1162-0) bit will be set again, and [BUFF_CPU_SHOULD_HANDLE](#page-1164-0) will change in this instance.
...
[SIE_STATUS](#page-1161-0).DATA_SEQ_ERROR (isochronous data is always sent with a DATA0 pid).
...
The USB device controller supports suspend, resume, and device-initiated remote resume (triggered with [SIE_CTRL](#page-1160-0).RESUME). There is an interrupt / status bit in [SIE_STATUS](#page-1161-0). It is not necessary to enable the suspend and resume interrupts, since suspend and resume are irrelevant to most devices.
...
The host needs to send keep-alive packets to the device every 1ms to keep the device from suspending. Full Speed mode uses a SOF (start of frame) packet. Low Speed mode uses an EOP (end of packet) instead. Set [SIE_CTRL](#page-1160-0).KEEP_ALIVE_EN and [SIE_CTRL.](#page-1160-0)SOF_EN to enable these packets.

Several bits in [SIE_CTRL](#page-1160-0) are used to begin a host transaction:
...
1. Send the IN token packet to the device. The target device address and endpoint come from the [ADDR_ENDP](#page-1158-0) register.

#### DATA phase:

- 1. Receive the first data packet from the device.
- 2. Raise RX timeout error if the device doesn't reply.
- 3. If this is *not* an Isochronous endpoint and the data PID does not match the buffer control register, raise [SIE_STATUS](#page-1161-0).DATA_SEQ_ERROR (isochronous data is always sent with a DATA0 pid).
...
- 1. Set the [BUFF_STATUS](#page-1162-0) bit and update the buffer control register.
...
1. Send an OUT packet to the device. The target device address and endpoint come from the [ADDR_ENDP](#page-1158-0) register.
...
- 1. Send the first data packet to the device. If the endpoint type is isochronous, there is no ACK phase, so the host controller goes straight to status phase. If ACK is received, go to status phase. Otherwise:
  - If the host receives no reply, raise [SIE_STATUS.](#page-1161-0)RX_TIMEOUT.
  - If the host receives NAK, raise [SIE_STATUS](#page-1161-0).NAK_REC and send the data packet again.
  - If the host receives STALL, raise [SIE_STATUS](#page-1161-0).STALL_REC and go to idle.

#### STATUS phase:

1. Set the [BUFF_STATUS](#page-1162-0) bit and update the buffer control register. FULL will be set to 0. TRANS_COMPLETE will be set if this is the last buffer in the transfer.
...
- Set the address and endpoint of the device in the appropriate ADDR_ENDP register ([ADDR_ENDP1](#page-1158-1) to [ADDR_ENDP15\)](#page-1158-1).
...
• Set the corresponding interrupt endpoint active bit (one of bits 1 through 15) in [INT_EP_CTRL](#page-1162-1).
...
The controller loops from 1 to 15 and attempts to poll any interrupt endpoint with the EP_ACTIVE bit set to 1 in [INT_EP_CTRL.](#page-1162-1) The controller will then read the endpoint control register and the buffer control register to see if there is an available buffer (i.e. FULL + AVAILABLE if an OUT transfer and NOT FULL + AVAILABLE for an IN transfer). If not, the controller will move onto the next interrupt endpoint slot.

If there is an available buffer, the transfer is dealt with the same as a normal IN or OUT transfer and the BUFF_DONE flag in [BUFF_STATUS](#page-1162-0) will be set when the interrupt endpoint has a valid buffer.
...
- **VBUS enable**, used to enable VBUS in host mode. Set in [SIE_CTRL.](#page-1160-0)
- **VBUS detect**, used to detect that VBUS is present in device mode. Set via a bit in [SIE_STATUS.](#page-1161-0) Can also raise a VBUS_DETECT interrupt enabled in [INTE.](#page-1173-0)
- **VBUS overcurrent**, used to detect an overcurrent event. Applicable to both device and host. VBUS overcurrent is a bit in [SIE_STATUS.](#page-1161-0)

It is not necessary to connect up any of these pins to GPIO. The host can permanently supply VBUS and detect a device being connected when either the DP or DM pin is pulled high. VBUS detect can be forced in [USB_PWR.](#page-1169-0)
