# <span id="page-1275-1"></span>**13.7. Device Decommissioning (RMA)**
...
Like all other lock words, the page 63 lock word is protected by its own locks, which means it can be hard- and softlocked to prevent the RMA flag being set. Locking the RMA flag makes it impossible to re-enable the factory JTAG interface if any of [CRIT1.](#page-1304-0)SECURE_BOOT_ENABLE, [CRIT1.](#page-1304-0)DEBUG_DISABLE or [CRIT1](#page-1304-0).SECURE_DEBUG_DISABLE is set. This makes it impossible for Raspberry Pi to re-test such devices if they are returned for fault analysis.
