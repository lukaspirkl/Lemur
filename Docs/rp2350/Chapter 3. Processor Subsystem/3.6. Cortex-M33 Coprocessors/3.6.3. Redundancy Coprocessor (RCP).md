# <span id="page-112-0"></span>**3.6.3. Redundancy Coprocessor (RCP)**
...
- Checking the valid state of the salt register, via [rcp_canary_status](#page-119-0)
- Writing a salt via [rcp_salt_core0](#page-119-1) or [rcp_salt_core1](#page-119-2), which writes a 64-bit value to that core's salt register, and changes its state to **valid**
...
- For mcr instructions (32-bit core-to-coprocessor):
  ...
  - If there is no 8-bit tag (i.e. any other than [rcp_canary_check](#page-120-0), [rcp_count_check](#page-122-2), [rcp_count_set](#page-122-1)), the CRn and CRm opcode fields must be all-zeroes.
- For mrc instructions (32-bit coprocessor-to-core):
  - The Opc1 field must be in the range 0 through 2.
  - For instructions other than [rcp_canary_get](#page-119-3) and [rcp_canary_check](#page-120-0), the CRn and CRm opcode fields must be allzeroes.
- For mcrr instructions (64-bit core-to-coprocessor):
  - The Opc1 field must be in the range 0 through 8.
  - For rcp_salt_core\* instructions, bits 2:0 of the CRm field must be 0 or 1 (referred to as [rcp_salt_core0](#page-119-1) and [rcp_salt_core1](#page-119-2) respectively).
...
- [rcp_canary_get](#page-119-3) generates a 32-bit value for an 8-bit tag as a function of the salt register
- [rcp_canary_check](#page-120-0) validates a 32-bit value for an 8-bit tag and raises an RCP fault if the value does not match that produced by an [rcp_canary_get](#page-119-3) for the same tag.
...
The LFSRs are implemented by shifting the XOR reduction of (state AND taps) into the LSB with each state update. When an LFSR's state is all-zeroes, a one bit is shifted into the LSB. The LFSR state advances each time a random number is generated: this happens when executing an instruction with a pseudorandom delay, or when executing a [rcp_random_byte](#page-122-3) instruction.
...
#### <span id="page-119-1"></span>**rcp_salt_core0**
...
#### <span id="page-119-2"></span>**rcp_salt_core1**
...
#### <span id="page-119-0"></span>**rcp_canary_status**
...
#### <span id="page-119-3"></span>**rcp_canary_get**
...
[Section 3.6.3.5](#page-116-0) specifies the 32-bit value returned by this instruction, but you should treat this as an opaque value to be consumed by rcp_canary_check.

#### <span id="page-120-0"></span>**rcp_canary_check**
...
#### <span id="page-122-0"></span>**rcp_iequal**
...
#### <span id="page-122-3"></span>**rcp_random_byte**
...
#### <span id="page-122-1"></span>**rcp_count_set**
...
#### <span id="page-122-2"></span>**rcp_count_check**
...
#### <span id="page-123-2"></span>**rcp_panic**
