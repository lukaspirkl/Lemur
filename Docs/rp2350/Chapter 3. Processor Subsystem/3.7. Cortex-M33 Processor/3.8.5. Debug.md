# <span id="page-285-0"></span>**3.8.5. Debug**
...
- Externally, through the system's SW-DP (see [Section 3.5\)](#page-84-2)
- Internally, via self-hosted debug (see [Section 3.5.6\)](#page-87-2)
...
Each Hazard3 core possesses exactly one hardware thread, or **hart**. This means each processor executes only a single stream of instructions at a time. The two Hazard3 processor cores on RP2350, core 0 and 1, have hart IDs of 0 and 1 respectively. These values can be read from the [MHARTID](#page-334-0) register on each processor, and match the values read from the [CPUID](#page-60-0) register in SIO.
...
- The Debug Module's data0 register is mapped into the core as a CSR, [DMDATA0](#page-333-0)
...
For more details on the core-side Debug mode registers, see [DCSR](#page-326-0) and [DPC.](#page-327-0)
