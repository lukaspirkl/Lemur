# <span id="page-341-0"></span>**4.4.1. XIP Cache**
...
You can disable cache lookup separately for Secure and Non-secure accesses via the [CTRL](#page-346-2).EN_SECURE and [CTRL](#page-346-2).EN_NONSECURE register bits. The [CTRL](#page-346-2) register contains controls to disable Secure/Non-secure access to the uncached and uncached/untranslated XIP windows, which avoids duplicate mappings that may otherwise require additional SAU or PMP regions.
...
By default, cache maintenance is Secure-only. Non-secure writes to the cache maintenance address window have no effect and return a bus error. Non-secure cache maintenance can be enabled by setting the [CTRL](#page-346-2).MAINT_NONSEC register bit, but this is not recommended if Secure software may perform cached XIP accesses.
...
Pinned cache lines are not accessible when the cache is disabled via the [CTRL](#page-346-2) register [\(CTRL](#page-346-2).EN_SECURE or [CTRL](#page-346-2).EN_NONSECURE depending on security level of the bus access).
