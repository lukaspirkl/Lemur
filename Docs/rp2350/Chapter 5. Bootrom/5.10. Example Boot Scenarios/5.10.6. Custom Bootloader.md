# <span id="page-435-0"></span>**5.10.6. Custom Bootloader**
...
- [get_partition_table_info\(\)](#page-389-0) to get the full partition information.
- or [get_partition_table_info\(\)](#page-389-0) with SINGLE_PARTITION, the chosen partition number, and PARTITION_LOCATION_AND_FLAGS, to get the address of a single partition
...
• [get_sys_info\(\)](#page-390-0) with BOOT_INFO, to get the flash_update_boot_window_base if any:
...
• [pick_ab_partition\(\)](#page-394-0) to pick the boot partition between A/B partitions if desired:
...
- or [get_b_partition\(\)](#page-389-1) to find the other partition directly.
- [chain_image\(\)](#page-383-2) with data_start_addr and data_size, to boot a chosen image:
