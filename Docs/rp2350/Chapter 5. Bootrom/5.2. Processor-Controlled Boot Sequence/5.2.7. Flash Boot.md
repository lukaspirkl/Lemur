# <span id="page-372-2"></span>**5.2.7. Flash Boot**
...
A flash image can be written into a partition of a [partition table](#page-354-2). The partition table is described by a PARTITION_TABLE block stored at the start of flash. The bootrom finds the partition table and scans its partitions to look for bootable images.

#### **[Partition-table-in-image boot](#page-360-1)**

A flash image containing an IMAGE_DEF and PARTITION_TABLE block in a single [block loop](#page-356-1) is written to the start of flash. The bootrom loads the embedded partition table, and enters the image in the same way as the [flash image boot](#page-359-1) case.
