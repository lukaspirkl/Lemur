# <span id="page-591-0"></span>**9.5. Interrupts**
...
There are enable, status, and force registers for three interrupt destinations: proc 0, proc 1, and dormant_wake. For proc 0 the registers are enable [\(PROC0_INTE0\)](#page-702-0), status ([PROC0_INTS0](#page-714-0)), and force [\(PROC0_INTF0\)](#page-708-0). Dormant wake is used to wake the ROSC or XOSC up from dormant mode. See [Section 6.5.6.2](#page-489-0) for more information on dormant mode.
...
Summary registers can be used to quickly check for pending GPIO interrupts. See [IRQSUMMARY_PROC0_NONSECURE0](#page-688-0) for an example.
