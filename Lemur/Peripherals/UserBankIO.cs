...
public class UserBankIO : PeripheralBase, IGpioSource
{
    ...
    private const int INTR_REG_COUNT = 6;
    private const int PINS_PER_INTR_REG = 8;
    private const uint EDGE_BIT_MASK = 0xCCCC_CCCC; // bits 2,3 of every 4-bit nibble

    // Latched edge interrupt bits. Only bits matching EdgeBitMask are meaningful.
    private readonly uint[] m_IntrEdgeLatch = new uint[INTR_REG_COUNT];
    ...
    private readonly uint[] m_Proc0Inte = new uint[INTR_REG_COUNT];
    private readonly uint[] m_Proc0Intf = new uint[INTR_REG_COUNT];
    private readonly uint[] m_Proc1Inte = new uint[INTR_REG_COUNT];
    private readonly uint[] m_Proc1Intf = new uint[INTR_REG_COUNT];
        ...
        : base(baseAddress, name, logger)
    {
        ...
        // -------------------------------------------------------------------------
        for (int i = 0; i < INTR_REG_COUNT; i++)
        {
            int idx = i;
    ...
    private void OnGpioLineChanged(int pinIdx, GpioValue newValue)
    {
        ...
        int regIdx    = pinIdx / PINS_PER_INTR_REG;
        int bitOffset = (pinIdx % PINS_PER_INTR_REG) * 4;
    ...
    private uint ReadIntr(int regIdx)
    {
        ...
        for (int j = 0; j < PINS_PER_INTR_REG; j++)
        {
            int pinIdx = regIdx * PINS_PER_INTR_REG + j;
            if (pinIdx >= 48) break;
    ...
    private void WriteIntrEdgeClear(int regIdx, uint value)
    {
        ...
        m_IntrEdgeLatch[regIdx] &= ~(value & EDGE_BIT_MASK);
        UpdateProc0Irq();
        // UpdateProc1Irq() — omitted until dual-core support is added.
    }
    ...
    private void UpdateProc0Irq()
    {
        ...
        for (int i = 0; i < INTR_REG_COUNT && !anyPending; i++)
