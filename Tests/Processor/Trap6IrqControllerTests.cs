...
///// <summary>
///// Chunk 6 — CsrController: Bridging 52 Peripheral IRQ Lines to MIP.MEIP
/////
...
/////
///// That bridge is CsrController. It works as follows:
/////
/////   • Peripherals call RaiseIrq(N) / ClearIrq(N) to assert/deassert IRQ N.
/////   • CsrController keeps a 64-bit pending mask (one bit per IRQ line).
...
///// CsrController from the same DI container.
///// </summary>
//public class Trap6CsrControllerTests
//{
...
//        var irq = sp.GetRequiredService<CsrController>();

//        emu.SetCSR(MEIEA_CSR, MeieaEnableValue(CsrController.IO_IRQ_BANK0));
//        irq.RaiseIrq(CsrController.IO_IRQ_BANK0); // assert IRQ 21
...
//        emu.SetCSR(MEIEA_CSR, MeieaEnableValue(CsrController.IO_IRQ_BANK0));
//        irq.RaiseIrq(CsrController.IO_IRQ_BANK0);
//        irq.ClearIrq(CsrController.IO_IRQ_BANK0);
...
//        // two writes).
//        emu.SetCSR(MEIEA_CSR, MeieaEnableValue(CsrController.IO_IRQ_BANK0));
//        emu.SetCSR(MEIEA_CSR, MeieaEnableValue(CsrController.UART0_IRQ));
//        irq.RaiseIrq(CsrController.IO_IRQ_BANK0); // GPIO (IRQ 21)
//        irq.RaiseIrq(CsrController.UART0_IRQ);    // UART0 (IRQ 33)

//        irq.ClearIrq(CsrController.IO_IRQ_BANK0); // GPIO cleared; UART0 still pending
